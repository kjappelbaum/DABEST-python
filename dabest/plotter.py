...
def EffectSizeDataFramePlotter(EffectSizeDataFrame, **plot_kwargs):
    ...
    **plot_kwargs:
    ...
    if show_pairs is True:
        ...
        swarm_subsample = plot_kwargs['swarm_subsample']
        if swarm_subsample:
            plot_data_swarm = plot_data.sample(frac=swarm_subsample)
        else:
            plot_data_swarm = plot_data
        rawdata_plot = sns.swarmplot(data=plot_data_swarm, x=xvar, y=yvar,
                                     ax=rawdata_axes,
    ...
    if float_contrast is True:
        ...
        if effect_size_type in ["mean_diff", "median_diff"]:
            ...
            scaled_ylim = ((rawdata_axes.get_ylim() -
                            control_group_summary) / ylim_scale_factor).tolist()
        ...
        rightend_ticks = np.array(
            [len(i)-1 for i in idx]) + np.array(ticks_to_skip)
